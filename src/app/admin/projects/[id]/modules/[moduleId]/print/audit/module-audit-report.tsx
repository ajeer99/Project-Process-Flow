...
import { FileText, Activity, Box } from 'lucide-react';
import { format } from 'date-fns';
import { PrintControls } from '../../../../../../components/print-controls';

export function ModuleAuditReport({ moduleData }: { moduleData: any }) {
                ...
                <div className="border-b-2 border-slate-900 pb-6 mb-8 flex justify-between items-end">
                    <div>
                        <div className="text-sm font-bold tracking-widest text-slate-500 uppercase flex items-center gap-1 mb-2">
                            {moduleData.project.name} <span className="text-slate-300 mx-1">•</span> Module Audit Log
                        </div>
                        <h1 className="text-4xl font-extrabold text-slate-900">{moduleData.name} Issue Log</h1>
                        <p className="text-slate-600 mt-2 max-w-2xl">Complete chronological log of all tracked issues assigned to this specific module.</p>
                    </div>
                    ...
                    <table className="w-full text-xs text-left">
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {bugs.length > 0 ? bugs.map((bug: any) => (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-4 py-8 text-center text-slate-500 text-sm">No issues recorded for this module.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
